...
public class PlayWorldOfMagic implements CommandLineRunner {
    ...
    public static int resolveBattle(
        ...
        for (int index = 0; index <= 2; index++) {
            float magiciandamage = spellDamageInfo[0];
            float bossHP = bossHPs[index];
            String bossName = bossNames[index];
            String magicianSpell = magicianSpells[0];
            while (bossHP > 0) {
                bossHP = bossHP - magiciandamage;
                spellsUsed++;
            }
        }
